...
class Agents:
    ...
    def choose_action(self, obs, last_action, agent_num, avail_actions, epsilon, maven_z=None, evaluate=False):
        ...
        if self.args.alg == 'coma' or self.args.alg == 'central_v' or self.args.alg == 'reinforce':
            ...
            # action = self._choose_action_from_hard(q_value.cpu(), avail_actions, epsilon, evaluate)
        else:
    ...
    def choose_action_full_input(self, input, agent_num, avail_actions, maven_z=None, evaluate=False):
        exp = []
        inputs = input
        ...
        if self.args.alg == 'coma' or self.args.alg == 'central_v' or self.args.alg == 'reinforce':
            ...
            if np.random.uniform() < epsilon:
                ...
                action = torch.argmax(q_value)
            exp.append(action.detach().numpy().tolist())
            exp.append(q_value[0].detach().numpy().tolist())
        return action, exp

    def choose_action_get_exp(self, obs, last_action, agent_num, avail_actions, epsilon, maven_z=None, evaluate=False):
        exp = []
        inputs = obs.copy()
        avail_actions_ind = np.nonzero(avail_actions)[0]  # index of actions which can be choose
        ...
        # transform the shape of inputs from (42,) to (1,42)
        inputs = torch.tensor(inputs, dtype=torch.float32).unsqueeze(0)
        inputs_avail_actions = inputs.tolist()[0] + avail_actions

        exp.append(inputs_avail_actions)  # ····················OUR
        # exp.append(obs.tolist())  # ····················VIPER
    ...
    def _choose_action_from_hard(self, inputs, avail_actions, epsilon, evaluate=False):
        epsilon = 0
        action_num = avail_actions.sum(dim=1, keepdim=True).float().repeat(1, avail_actions.shape[
            -1])  # num of avail_actions
        # 先将Actor网络的输出通过softmax转换成概率分布
